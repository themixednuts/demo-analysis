...
export class DemoAnalyzer {
  ...
  decodeNetworkPacket(
    ...
    try {
      ...
      const decoder = REGISTRY_LOOKUP.get(type);
      if (decoder) {
        return { success: true, data: decoder(type, data) };
      }
    ...
    while (bitbuffer.remaining_bytes() > 0) {
      ...
      try {
        ...
        if (bitbuffer.remaining_bits() < 4) {
          packet.error =
            `Not enough bits for packet type (${bitbuffer.remaining_bits()} < 4)`;
          packets.push(packet);
          break;
        }
        ...
        if (packet.size > bitbuffer.remaining_bytes()) {
          packet.error =
            `Size ${packet.size} > remaining bytes ${bitbuffer.remaining_bytes()}`;
          packets.push(packet);
          break;
        }
  ...
  analyzeChunk(chunk: BroadcastChunk, index: number): ChunkAnalysis {
    ...
    const { command, isCompressed, tick } = chunk.header();
    const rawCommand = command |
      (isCompressed ? Demo.EDemoCommands.DEM_IsCompressed : 0);
    ...
    if (networkPacketChunks.includes(chunk.command)) {
      ...
      try {
        analysis.networkPackets = this.analyzeNetworkPackets(
          data,
          index,
          chunk.command,
        );
  ...
  analyzeFragment(buffer: ArrayBuffer): DemoAnalysis {
    ...
    chunks.forEach((chunk, index) => {
      ...
      if (chunkAnalysis.analysisType === "network_packets") {
        ...
        if (chunkAnalysis.networkPackets) {
          analysis.summary.totalNetworkPackets +=
            chunkAnalysis.networkPackets.length;
    ...
    if (analysis.chunks.length > 0) {
      ...
      analysis.performance.averageChunkParseTimeMs = chunkTimes.reduce((a, b) =>
        a + b, 0) / chunkTimes.length;
...
export class OutputFormatter {
  ...
  static toConsole(analysis: DemoAnalysis): void {
    ...
    console.log(
      `Network Packet Chunks: ${analysis.summary.networkPacketChunks}`,
    );
    console.log(`Protobuf Chunks: ${analysis.summary.protobufChunks}`);
    console.log(`Unknown Chunks: ${analysis.summary.unknownChunks}`);
    console.log(
      `Total Network Packets: ${analysis.summary.totalNetworkPackets}`,
    );
    ...
    analysis.chunks.forEach((chunk) => {
      ...
      if (chunk.networkPackets && chunk.networkPackets.length > 0) {
        ...
        chunk.networkPackets.forEach((packet) => {
          console.log(
            `    ${packet.packetIndex}: ${packet.typeName} (${packet.type}) - ${packet.size} bytes${
